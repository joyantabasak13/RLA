...
int main(int argc, char** argv) {
    ...
    if (coreRank == ROOT_ID) {
		...
		getCombinedData();
		getCombinedDataSuperString();
	}

	MPI_Bcast(&totalRecords, 1, MPI_INT, ROOT_ID, MPI_COMM_WORLD);
	MPI_Bcast(&lenMax, 1, MPI_INT, ROOT_ID, MPI_COMM_WORLD);
	...
	if(coreRank != ROOT_ID) {
		populateCombinedRecordsFromSuperString();
		int numRecToBeSorted = ceil(totalRecords/l);
		int start = numRecToBeSorted*(coreRank-1);
		int end = start + numRecToBeSorted;
		end = end > totalRecords? totalRecords: end;
		combinedDataToBeSorted.resize(end-start);
		for(size_t i= start; i<end; i++){
			combinedDataToBeSorted[i] = combinedData[start+i];
		}
		radixSort(combinedDataToBeSorted);
		// divide data to m parts
		int perMIndices = ceil(totalRecords/(l*m));
		myIndicesToSort = new int[perMIndices*l];
		int *indicesToSort = new int[perMIndices];
		mWayShuffledIndices = new int*[m];
		for(size_t i = 0; i<m; i++){
			mWayShuffledIndices[i] = new int[perMIndices];
		}
		for(size_t i = 0; i<combinedDataToBeSorted.size(); i++){
			mWayShuffledIndices[i%m][floor(i/m)] = combinedDataToBeSorted[i].first;
		}
		for(size_t i=combinedDataToBeSorted.size(); i<m*perMIndices; i++){
			mWayShuffledIndices[i%m][floor(i/m)] = -1;
		}
		for(size_t i = 0; i<l; i++){
			if(i+1 != coreRank){
				MPI_Isend(&mWayShuffledIndices[i], perMIndices, MPI_INT, i+1, SHUFFLED_IND_ARR, MPI_COMM_WORLD);
			}
		}
		for(size_t i = 0; i<l; i++){
			if(i+1 != coreRank){
				MPI_Recv(&indicesToSort, 1, MPI_INT, i+1, SHUFFLED_IND_ARR, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			}
		}
	}
