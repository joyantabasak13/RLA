...
int jaroDistanceLinear(string& s1, string& s2){
	...
	for (int i = 0; i < len1; i++) {
		if (s1MatchAtPos[i])
		{
			while (s2MatchAtPos[point] == 0 && point<len2)
				point++;
...
int getDistance(vector<string>& vec1, vector<string>& vec2){
	...
	for(int i=1; i<attributes-1; i++){
		dist = jaroDistanceLinear(vec1[i], vec2[i]);
...
int main(int argc, char** argv) {
	...
	string completeLinkageClusterFile = "JaroLINEARLinked_" + singleLinkageFileName;
