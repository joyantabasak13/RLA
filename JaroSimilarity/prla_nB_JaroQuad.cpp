...
double jaroDistance_1(string &s1, string &s2, int id)
{
...
{
	// cout<< "Is linkage ok between: " << a[blockingAttrIndex] << " and " << b[blockingAttrIndex] << endl;
	double blockField_dist;
	for(int i=1; i<attributes; i++){
		blockField_dist = jaroDistance_1(a[i], b[i], id);
...
double getLinkageValue(vector<string> &a, vector<string> &b, double threshold){
	...
	for(int i=1; i<attributes; i++){
		blockField_dist = jaroDistance_1(a[i], b[i], 0);
...
int main(int argc, char** argv) {
	...
	string out_name1 = out_file_path + "out_single_linkage_"+ fileName + "_JaroSimilarity_" + to_string(distanceThreshold);
	string out_name2 = out_file_path + "out_complete_linkage_"+ fileName + "_JaroSimilarity_" + to_string(distanceThreshold);
	string stat_file_name = "stat_"+ fileName + "_JaroSimilarity_" + to_string(distanceThreshold);
