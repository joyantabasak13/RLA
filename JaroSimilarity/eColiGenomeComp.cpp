...
double jaroDistanceLinear(string& s1, string& s2){
	...
	t = t / 2.0;
	// cout<< "Match: " << match << " len1: " << len1 << " len2 " << len2 << " transpos " << t << endl;
...
void getData(string& file_path) {
	...
	while(records >> line) {
        recordVector.push_back(line);
		if(line.size() > largest) {
			largest = line.size();
		} else if (line.size() < smallest) {
			smallest = line.size();
		}
		totalChar += line.size();
    }
...
int main(int argc, char **argv)
{
	...
	memset(s1MatchAtPos, 0, stringMaxLen * sizeof(int));
	memset(s2MatchAtPos, 0, stringMaxLen * sizeof(int));
	for(int i=0; i<recordVector.size()-1; i++){
		for(int j=i+1; j<recordVector.size(); j++) {
			...
			compCount++;
			// if(compCount==1000000) {
			// 	// cout<< compCount << " times comparision done" << endl;
			// 	// double compTime_temp	= (double)(clock() - currTS_l1) / CLOCKS_PER_SEC;
    		// 	// cout<< "CompDone " << compCount << " Took Time " << compTime_temp << endl;
			// 	goto breakLoop1;
			// }
		}
	}
	breakLoop1:
	cout<< "Total comp: " << compCount<< endl;
	return 0;
	double compTime	= (double)(clock() - currTS_l1) / CLOCKS_PER_SEC;
    cout<< "Linear algo Total Time " << compTime << endl;
	...
	// 		compCount++;
	// 		if(compCount%2000000 == 0) {
	// 			cout<< compCount << " times comparision done" << endl;
	// 		}
	// 	}
	// }
	// double quadtotalCompTime	= (double)(clock() - currTS_q1) / CLOCKS_PER_SEC;
    // cout<< "QuadAlg 1 Total Time " << quadtotalCompTime << endl;
	...
	breakLoop2:
	double quad2totalCompTime	= (double)(clock() - currTS_q2) / CLOCKS_PER_SEC;
	cout<< "QuadAlg 2 Total Time " << quad2totalCompTime << endl;
