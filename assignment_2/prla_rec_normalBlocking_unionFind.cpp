...
map<int, set<int>> blocksToRecords;
map<int, set<int>> recordsToBlocks;
void findBlocksRecordsCorrespondence() {
	for(int i = 0; i< blockingIDList.size(); i++) {
		int blockID = blockingIDList[i].first;
		int recID = blockingIDList[i].second;
		blocksToRecords[blockID].insert(recID);
		recordsToBlocks[recID].insert(blockID);
	}
}

map<int, int> recordCompCount;
void getRecordCompCount(){
	long long int totalRecComp = 0;
	double averageClustersPerRecord = 0;
	for(int r =0; r<totalUniqueRecords; r++) {
		set<int> r_comp_recs;
		set<int> r_blocks = recordsToBlocks[r];
		set<int>:: iterator it_b;
		averageClustersPerRecord += r_blocks.size();
		continue;
		for( it_b = r_blocks.begin(); it_b!=r_blocks.end(); ++it_b){
			int blockID = *it_b;
			set<int> b_recs = blocksToRecords[blockID];
			set<int>:: iterator it_r;
			for( it_r = b_recs.begin(); it_r!=b_recs.end(); ++it_r){
				int recID = *it_r;
				if (recID < r) {
					r_comp_recs.insert(recID);
				}			
			}
		}
		recordCompCount[r] = r_comp_recs.size();
		totalRecComp = totalRecComp + r_comp_recs.size();	
	}
	cout<< "Average number of clusters per record:" << (double) averageClustersPerRecord / totalUniqueRecords << endl;
	cout<< "Total Number of Unique Comparisions: " << totalRecComp << endl;
}
...
int main(int argc, char** argv) {
	...
	findBlocksRecordsCorrespondence();
	getRecordCompCount();
