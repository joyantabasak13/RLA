...
void getDistMat(){
	...
	for (int i = 0; i < n-1; ++i) {
		...
		// cout<< "All pair dist calc done" << endl;
		vector<pair<int,int>> minEdges;
		// Extract the mins
		minDist = nnPQ_i.top().first;
		while(minDist >= nnPQ_i.top().first) {
			minEdges.push_back(nnPQ_i.top().second);
			nnPQ_i.pop();
			if(nnPQ_i.empty()){
				break;
			}
		}
		// cout<< "Got minimum neighbours: " << minEdges.size() << " for i: " << i << endl;
		// this adds some bias if the sizeof minEdges is close to RandMax() which is not expected in our case
		int randMinIndex = rand() % minEdges.size();
		// cout<< "Selected RandMinEdgeIndex: " << randMinIndex << endl;
		pair<double, pair<int,int> > edge;
		edge.first = (double)((double)minDist);
		edge.second = minEdges[randMinIndex];
		// cout<<"Selected NN edge u: " << edge.second.first << " v: " << edge.second.second << endl;
		pq.push(edge);
	}
}
			...
			if(isAdded) {
				...
				if(cluster.sources.size() == numSources) {
					...
					for (const auto &p : clusterIdToCluster){
						...
						if (dist < minDist)
						{
							// need same random min distance strategy
							minDist = (double)dist;
							minInd = p.first;
							// cout<< "Cluster: " << clusterID << " Has potential edges" << endl;
						}
					...
					if (minInd != -1) {
						...
						edge.second.second = minInd;
						edgesToAdd.push_back(edge);
						// pq.push(edge);
						// cout<< "Edge u: " << clusterID << " v: " << minInd << " is added to PQ" << endl;
					}
...
int main(int argc, char** argv) {
	...
	string completeLinkageClusterFile = "Complete_Linked_" + singleLinkageFileName;
